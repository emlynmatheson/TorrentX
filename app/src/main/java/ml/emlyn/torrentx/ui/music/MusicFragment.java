...
public class MusicFragment extends Fragment {
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        ((BottomNavigationView) requireView().findViewById(R.id.bt_nav_music)).setOnNavigationItemSelectedListener(
                item -> {
                    switch (item.getItemId()) {
                        case R.id.download_menu: {
                            ...
                            ((EditText) requireActivity().findViewById(R.id.music_dl_search_bar)).setText("");
                            requireActivity().findViewById(R.id.music_dl_search).setOnClickListener(this::dlSearchBtnOnClick);

                            ((LinearLayout) requireActivity().findViewById(R.id.music_dl_ll)).removeAllViews();
                            requireActivity().findViewById(R.id.musicNoResFoundIV).setVisibility(INVISIBLE);
                            requireActivity().findViewById(R.id.musicNoResFoundTV).setVisibility(INVISIBLE);
                        ...
                        case R.id.open_menu: {
                            ...
                            ((EditText) requireActivity().findViewById(R.id.music_dl_search_bar)).setText("");
                            requireActivity().findViewById(R.id.music_dl_search).setOnClickListener(this::opSearchBtnOnClick);
        ...
        (requireView().findViewById(R.id.music_dl_search)).setOnClickListener(this::dlSearchBtnOnClick);

        EditText searchEdit = requireView().findViewById(R.id.music_dl_search_bar);
    ...
    private void dlUpdateSearchRes() {
        ...
        new Thread(() -> torrents.searchTorrent(((EditText) requireView().findViewById(R.id.music_dl_search_bar)).getText().toString(), "music", requireActivity(), searchRes -> {
            if (searchRes.length == 0 && !((EditText) requireView().findViewById(R.id.music_dl_search_bar)).getText().toString().equals("")) {
                requireActivity().findViewById(R.id.musicNoResFoundIV).setVisibility(VISIBLE);
                requireActivity().findViewById(R.id.musicNoResFoundTV).setVisibility(VISIBLE);
            }

            for (int i=0; i<searchRes.length; i++) {
                String[] res = searchRes[i];
                createEntry(res[0], i, res[2], res[1], requireActivity().findViewById(R.id.music_dl_ll));
            }
        })).start();
    }
